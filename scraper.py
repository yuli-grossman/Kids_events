...
SPECIAL_NEEDS_ONLY_PATTERNS = [
    r'לילדים?\s+(?:עם|בעלי)\s+(?:צרכים\s*מיוחדים|מוגבלות|מוגבלויות|מגבלה|אוטיזם|לקויות)',
    r'ילדי[םה]?\s+(?:ה?ב|עם\s+ה?)ספקטרום',
    r'\bחינוך\s+מיוחד\b',
    r'כיתות?\s+מיוחדות?',
    ...
    r'ילד[יה.ה]?\s+עם\s+(?:מוגבלות|מוגבלויות|מגבלה|אוטיזם|לקויות)',
    r'(?:^|\s)לבעלי\s+(?:צרכים|מוגבלות|מגבלה)',
    r'בעלי\s+(?:מוגבלויות?|מגבלה)',
    r'(?:עם|בעלי)\s+מגבלה\b',          # "עם מגבלה", "בעלי מגבלה"
